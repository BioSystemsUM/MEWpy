...
##################################################################
"""
from mewpy.optimization.problem import RKOProblem, ROUProblem, GeckoRKOProblem, GeckoROUProblem , GKOProblem, GOUProblem
...
def cb_ko(product, chassis = 'ec', display=False, filename = None):
    ...
    evaluator_1 = BPCY(BIOMASS_ID, PRODUCT_ID, method = SimulationMethod.lMOMA )
    evaluator_2 = WYIELD(BIOMASS_ID,PRODUCT_ID)
    problem = GKOProblem(model, fevaluation=[evaluator_1,evaluator_2], non_target=non_target, envcond=envcond, reference = reference)
