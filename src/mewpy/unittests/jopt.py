...
CB model optimization test Set of using jmetalpy EA module

##################################################################
"""
from mewpy.optimization.jmetal.problem import RKOProblem, ROUProblem
...
def load_yeast():
        ...
        NON_TARGET_FILE = os.path.join(PATH, "nontargets#RK#iMM904SL_v6#[lim-aerobic#glucose].txt")
...
def cb_ou(product, chassis = 'ec', display=False, filename = None):
    ...
    problem = ROUProblem(model, fevaluation=[evaluator_1,evaluator_2], non_target=non_target, envcond=envcond, reference = reference)
...
def cb_ko(product, chassis = 'ec', display=False, filename = None):
    ...
    evaluator_1 = BPCY(BIOMASS_ID, PRODUCT_ID, method = SimulationMethod.lMOMA )
    evaluator_2 = WYIELD(BIOMASS_ID,PRODUCT_ID)
    problem = RKOProblem(model, fevaluation=[evaluator_1,evaluator_2], non_target=non_target, envcond=envcond, reference = reference)

    ea = EA(problem, max_generations= ITERATIONS, mp = True , visualizer= False)
    final_pop = ea.run()
...
if __name__ == '__main__':
    ...
    for k,v in compounds_EC.items():
       for i in range(RUNS):
            ...
            cb_ko(v,filename= "CBMODEL_EC_{}_KO_{}.csv".format(k,millis))

    
    for k,v in compounds_EC.items():
        for i in range(RUNS):
            millis = int(round(time() * 1000))
            cb_ou(v,filename= "CBMODEL_EC_{}_OU_{}.csv".format(k,millis))
    

    for k,v in compounds_YS.items():
        for i in range(RUNS):
            millis = int(round(time() * 1000))
            cb_ko(v,chassis='ys',filename= "CBMODEL_YEAST_{}_KO_{}.csv".format(k,millis))

    for k,v in compounds_YS.items():
        for i in range(RUNS):
            millis = int(round(time() * 1000))
            cb_ou(v,chassis='ys',filename= "CBMODEL_YEAST_{}_OU_{}.csv".format(k,millis))
